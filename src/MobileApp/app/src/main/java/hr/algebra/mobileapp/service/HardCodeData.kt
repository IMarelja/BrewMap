 ...
 *
 * ## Ratings
 * Locations and drinks are always seeded with `averageRating = 0` /
 * `totalReviews = 0` (and `AggregatedRating(0.0, 0)` for drinks). The aggregates
 * are **derived** from the seed [reviews] via [pushReview] in the `init` block —
 * the exact same path the review service uses when a new review is added at
 * runtime. This guarantees a location/drink with no reviews shows `0`, and that
 * the average always matches the reviews that actually exist.
 */
class HardCodeData {
     ...
     * All locations start with a zero rating — the aggregate is built from [reviews] on init.
     */
    val locations: MutableList<Location> = mutableListOf(
        ...
        loc(
            ...
            hours = hoursClosedSunday("08:00", "20:00")
        ),
        loc(
        ...
        loc(
            ...
            hours = hoursClosedSunday("06:30", "18:00")
        ),
        loc(
            ...
            categoryTag = "cafe", paymentTags = listOf("cash", "card"),
            hours = hoursClosedSunday("07:00", "19:00")
        ),
        loc(
    ...
    val drinks: MutableList<Drink> = mutableListOf(
        ...
        drink("hc-drk-001", "Ožujsko Pivo",       "Classic Croatian lager on tap.",    "hc-loc-001"),
        drink("hc-drk-002", "Karlovačko Tamno",   "Dark lager with a malty finish.",   "hc-loc-001"),
        drink("hc-drk-003", "Aperol Spritz",       "Refreshing Italian aperitif.",      "hc-loc-001"),

        // Vinkl (hc-loc-002) — 2 drinks
        drink("hc-drk-004", "Flat White",          "Double ristretto with steamed milk.", "hc-loc-002"),
        drink("hc-drk-005", "Craft IPA",           "Hoppy local India pale ale.",        "hc-loc-002"),

        // Botaničar (hc-loc-003) — 2 drinks
        drink("hc-drk-006", "Cold Brew",           "12-hour cold-extracted coffee.",    "hc-loc-003"),
        drink("hc-drk-007", "Gin & Tonic",         "Premium gin with Fever-Tree tonic.", "hc-loc-003"),

        // Caffe Bar SKA (hc-loc-004) — 2 drinks
        drink("hc-drk-008", "Espresso",            "Short, intense double shot.",       "hc-loc-004"),
        drink("hc-drk-009", "Cappuccino",          "Classic Italian-style cappuccino.", "hc-loc-004"),

        // Leggiero Malešnica (hc-loc-007) — 2 drinks
        drink("hc-drk-010", "Cortado",             "Equal parts espresso and warm milk.", "hc-loc-007"),
        drink("hc-drk-011", "Oat Milk Latte",      "Espresso with steamed oat milk.",  "hc-loc-007"),

        // Mr. Jack Bar (hc-loc-008) — 2 drinks
        drink("hc-drk-012", "Jack Daniel's Cola",  "Bourbon and cola over ice.",        "hc-loc-008"),
        drink("hc-drk-013", "Whiskey Sour",        "Whiskey, lemon juice, simple syrup.", "hc-loc-008"),
        ...
        drink("hc-drk-017", "Matcha Latte",        "Ceremonial matcha with oat milk.", "hc-loc-010")
    )
     ...
     *
     * These are the single source of truth for ratings: the `init` block replays
     * each one through [pushReview], which is what populates [Location.averageRating] /
     * [Location.totalReviews] and [Drink.aggregatedRating]. A location or drink with
     * no review here therefore stays at `0`.
     *
    ...
    val reviews: MutableList<Review> = mutableListOf(
        ...
        // Ožujsko Pivo (hc-drk-001) — 3 reviews → avg 4.67
        ...
        // Espresso (hc-drk-008) — 2 reviews → avg 4.5
        ...
        // Vanilla Latte (hc-drk-014) — 2 reviews → avg 4.5
        ...
        // Specialty Filter (hc-drk-016) — 2 reviews → avg 4.5
    ...
    init {
        // Derive every location/drink rating from the seed reviews above, using the
        // exact same path the review service calls at runtime. Seeds start at 0, so
        // anything without a review here correctly stays at 0.
        reviews.forEach { pushReview(it) }
    }

    /**
     * Fold a single review into the aggregate of its target location or drink:
     * recompute the running average and increment the review count by one.
     *
     * If the review targets an id that is not in [locations] / [drinks], the review
     * is left untouched but the aggregate update is skipped and an error is logged —
     * the review simply contributes to no aggregate.
     *
     * Called once per seed review on init, and again by
     * [hr.algebra.mobileapp.service.review.ReviewServiceHardCode] whenever a new
     * review is created at runtime.
     */
    fun pushReview(review: Review) {
        when (review.targetType) {
            "location" -> pushLocationRating(review.targetId, review.rating)
            "product"                     -> pushDrinkRating(review.targetId, review.rating)
            else -> Log.e(
                TAG,
                "pushReview: review '${review.id}' has unknown targetType " +
                "'${review.targetType}'; rating aggregate not updated"
            )
        }
    }

    private fun pushLocationRating(locationId: String, rating: Int) {
        val index = locations.indexOfFirst { it.id == locationId }
        if (index == -1) {
            Log.e(TAG, "pushReview: location '$locationId' not found; rating ignored")
            return
        }
        val loc = locations[index]
        val newTotal = loc.totalReviews + 1
        val newAverage = roundToTwo((loc.averageRating * loc.totalReviews + rating) / newTotal)
        locations[index] = loc.copy(averageRating = newAverage, totalReviews = newTotal)
    }

    private fun pushDrinkRating(drinkId: String, rating: Int) {
        val index = drinks.indexOfFirst { it.id == drinkId }
        if (index == -1) {
            Log.e(TAG, "pushReview: drink '$drinkId' not found; rating ignored")
            return
        }
        val drink = drinks[index]
        val agg = drink.aggregatedRating
        val newCount = agg.count + 1
        val newAverage = roundToTwo((agg.average * agg.count + rating) / newCount)
        drinks[index] = drink.copy(aggregatedRating = AggregatedRating(newAverage, newCount))
    }

    private fun roundToTwo(value: Double): Double = Math.round(value * 100.0) / 100.0
    ...
    private companion object {
        private const val TAG = "HardCodeData"
    }
}
