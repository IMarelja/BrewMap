...
function AddDrinkContent() {
  ...
  const [formData, setFormData] = useState({ name: '', description: '', locationId: locationId || '' })

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    try {
      await api.post('/api/Drink', { name: formData.name, description: formData.description, locationId: formData.locationId })
    ...
    <ProtectedRoute>
      <div style={{ background: '#F9F3E9', minHeight: '100vh', paddingBottom: '4rem' }}>
        ...
        <div style={{ maxWidth: '700px', margin: '0 auto', padding: '2rem' }}>
          <h1 style={{ fontSize: '2.5rem', color: '#2C1A0E' }}>Add a New Drink</h1>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button type="submit" disabled={loading} style={primaryBtn}>{loading ? 'Adding...' : '+ Add Drink'}</button>
            </div>
          </form>
        </div>
      </div>
